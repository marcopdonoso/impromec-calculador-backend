...
export class TrayCalculatorService {
  ...
  async calculateOptimalTray(
    ...
    if (installationLayer === 'singleLayer') {
      ...
      console.log(
        `Disposición detectada: ${horizontalArrangement ? 'Horizontal' : 'Trébol'}`,
      );

      // Calcular el área real de los cables sin factores adicionales
      const rawCablesArea = this.calculateTotalArea(cablesInTray);
      console.log(
        `Área real ocupada por los cables (sin factores): ${rawCablesArea}mm²`,
      );
      ...
      if (horizontalArrangement) {
        ...
        console.log(
          `Dimensiones horizontales calculadas: ${requiredWidth}mm x ${requiredHeight}mm = ${calculatedArea}mm²`,
        );
      } else {
        ...
        calculatedArea = dimensions.width * dimensions.height;
        console.log(
          `Dimensiones trébol calculadas: ${requiredWidth}mm x ${requiredHeight}mm = ${calculatedArea}mm²`,
        );
      }
      ...
      requiredArea = this.calculateRequiredArea(
        cablesInTray,
        reservePercentage,
      );
  ...
  private calculateSingleLayerHorizontalDimensions(
    ...
    totalWidth =
      totalWidth * this.DIMENSION_SAFETY_FACTOR * (1 + reservePercentage / 100);

    // Altura = diámetro del cable más grande
    let maxHeight = Math.max(
      ...cablesInTray.map((cable) => cable.cable.externalDiameterMM),
    );
  ...
  private calculateSingleLayerCloverDimensions(
    ...
    for (const [diameterKey, cables] of Object.entries(cablesByDiameter)) {
      ...
      const totalQuantity = cables.reduce(
        (sum, cable) => sum + cable.quantity,
        0,
      );
      ...
      console.log(
        `- Diámetro ${diameter}mm: ${totalQuantity} cables = ${cloverGroups} grupos x 2 x ${diameter}mm = ${groupWidth}mm`,
      );
    ...
    // Aplicar factor de seguridad y reserva al ancho
    const widthWithFactors =
      totalWidth * this.DIMENSION_SAFETY_FACTOR * (1 + reservePercentage / 100);
    console.log(
      `Ancho con factores: ${totalWidth} x ${this.DIMENSION_SAFETY_FACTOR} x (1 + ${reservePercentage}/100) = ${widthWithFactors}mm`,
    );
    ...
    console.log(
      `Área rectangular resultante: ${totalWidth} x ${maxHeight} = ${resultingArea}mm²`,
    );
   ...
   */
  private groupCablesByDiameter(
    cablesInTray: CableInTray[],
  ): Record<string, CableInTray[]> {
  ...
  private async findSuitableTraysForSingleLayer(
    ...
    const suitableTrays = await this.cableTrayModel
      .find({
        type: trayType,
  ...
  private async findSuitableTraysForMultiLayer(
    ...
    const traySizesByLoad = await this.cableTrayModel
      .find({
        type: trayType,
        loadCapacity: { $gte: requiredLoadCapacity },
        isActive: true,
    ...
    return traySizesByLoad.filter((tray) => {
      ...
      if (trayType === 'escalerilla') {
        ...
        const usefulHeight =
          tray.height -
          (tray.type === 'escalerilla' ? this.LADDER_HEIGHT_REDUCTION : 0);
  ...
  private calculateUsefulArea(tray: CableTray): number {
    if (tray.type === 'escalerilla') {
      ...
      const usefulHeight = Math.max(
        0,
        tray.height - this.LADDER_HEIGHT_REDUCTION,
      );
  ...
  private createResultsFromTrays(
    ...
    const alternativeTrays = sortedTrays
      .slice(1, 4)
      .map((tray) => this.convertToTrayModel(tray));
    console.log(
      `${alternativeTrays.length} bandejas alternativas seleccionadas`,
    );
  ...
  private convertToTrayModel(catalogTray: CableTray): Tray {
    ...
    if (!trayCategory) {
      for (const [key, value] of Object.entries(categoryMap)) {
        if (
          ...
          trayCategory = value;
          console.log(
            `Coincidencia parcial encontrada: '${key}' -> '${value}'`,
          );
          break;
        }
      }
    ...
    if (!trayCategory) {
      console.log(
        'No se encontró coincidencia para la categoría, usando valor por defecto: liviana',
      );
